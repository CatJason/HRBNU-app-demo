...
public class Pendulum extends View {
    ...
    private int number = 7; //球的个数（奇数，偶数感觉丑就没实现）
    private static int angle = 40;//最大旋转角度
    ...
    private float t = 90f;//时间；可以用来控制速率，t越小，摆钟越慢；t越大，摆钟越快
        ...
        if (number % 2 == 1) {
            ...
            for (int i = -leftNumber; i <= leftNumber; i++) {
                ...
                if(i==0){
                    ...
                    linePath.lineTo(x, -(r*(number-2) + length));
                    //绘制直线
                    linePaint.setStyle(Paint.Style.FILL_AND_STROKE);
                    canvas.drawPath(linePath, linePaint);
                    //绘制圆圈，为了不重合，使用FILL，不绘制线的宽度
                    linePaint.setStyle(Paint.Style.FILL);
                    canvas.drawCircle(x, 0, r*(number-2), linePaint);
                }

                if(i==(-leftNumber)||i==(leftNumber)){
                    //计算圆心的横坐标x
                    x = 2 * r * i;
                    if (linePath == null)
                        linePath = new Path();
