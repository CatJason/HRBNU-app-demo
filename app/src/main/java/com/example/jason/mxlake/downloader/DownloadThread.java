...
public class DownloadThread extends Thread {
	...
	public DownloadThread(FileDownloadered downloader, URL downUrl, File saveFile, int block, int downLength, int threadId,int threadNum) {
	...
	public void run() {
		if(downLength < block){//未下载完成
			try {
                ...
                if(threadId%threadNum==0){
                    startPos = 0;
                }else{
                    startPos = block * (threadId%threadNum) + downLength;//开始位置
                    Log.i("gggggg",block+"*"+threadId+"+"+downLength);
                }
                ...
                Log.i("tttt",threadId+"%"+threadNum);
				//用户没有要求停止下载,同时没有达到请求数据的末尾时会一直循环读取数据
				while (!downloader.getExited() && (offset = inStream.read(buffer, 0, 1024)) != -1&&(downLength<(endPos-startPos))){
